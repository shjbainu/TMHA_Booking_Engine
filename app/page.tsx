...
import { AmenitiesContent } from "@/components/amenities-content"
...
export default function HotelPhotosPage() {
  ...
  const getBottomSheetContent = () => {
    switch (activeBottomSheet) {
      case 'overview':
        return { title: 'Tổng quan', content: <OverviewContent /> }
      case 'amenities':
        return { title: 'Tiện ích', content: <AmenitiesContent /> }
      case 'reviews':
        return { title: 'Đánh giá', content: <ReviewsContent /> }
      case 'more':
        return { title: 'Xem thêm', content: <MoreContent /> }
      default:
        return { title: '', content: null }
    }
  }

  const { title, content } = getBottomSheetContent()
